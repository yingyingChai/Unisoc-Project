namespace Spreadtrum.LHD.Business
{
    ...
    public static class UserService
    {
        ...
        public static void CreateSPRDUser(UserRoles userRole, string thirdPartyLoginAccount, string BUName, string fullname, string email, bool active, string operateUser)
        {
        ...
        public static User GetOSATUserByHashedCID(string hashedCID)
        {
            ...
            return new User { 
                AccountState = 1, AccountType = 0, BUName = user.SupID, CreateTime = Convert.ToDateTime("2016-01-01"), Email = user.SupMail, FullName = user.SupRealUser, LastOperator = "SYSTEM", LoginPassword = "", RecordState = 0, Remarks = "", Role = UserRoles.OSAT, ThirdpartyAccountID = user.CID.ToString(), ThirdpartyLoginProvider = "LHD LINKED SERVER", UpdateTime = DateTime.Now, UserID = user.MD5CID, WaitForConfirm = user.WaitForConfirm, 
                WaitForDispose = 0, WaitForOtherBinDispose = 0, NewComments = user.NewComments, EQALots = user.EQALots
             };
        }
        ...
        public static IList<User> GetUsersBy(string keyword, string email, string role, string accountState, int pageIndex, int pageSize, out int recordCount)
        {
            return userGateway.GetUsersBy(keyword, email, role, accountState, pageIndex, pageSize, out recordCount);
        }

        public static IList<User> GetUsersByRole(UserRoles role, int pageIndex, int pageSize, out int recordCount)
        {
            return userGateway.GetUsersByRole(role, pageIndex, pageSize, out recordCount);
        }
