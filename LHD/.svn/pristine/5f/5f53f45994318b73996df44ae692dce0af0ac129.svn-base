...
SWFUpload.prototype.initSettings = function (userSettings) {
	...
	this.ensureDefault("button_width", 2);
...
SWFUpload.prototype.getFlashHTML = function (flashVersion) {
    ...
    return ['<object ' + iefix + 'id="', this.movieName, '" type="application/x-shockwave-flash" wmode="transparent" data="', (this.support.imageResize ? this.settings.flash_url : this.settings.flash9_url), '" width="', this.settings.button_width, '" height="', this.settings.button_height, '" class="swfupload">',
...
SWFUpload.prototype.callFlash = function (functionName, argumentArray) {
    ...
    try {
        if (movieElement != undefined) {
            var instruction = '<invoke name="' + functionName + '" returntype="javascript">' + __flash__argumentsToXML(argumentArray, 0) + '</invoke>';
            returnString = movieElement.CallFunction(instruction);
...
function CreateUploader(buttonID, buttonText, buttonStyle, promptLayer, handler, successClientHandler) {
        uploaderControl = null;
         
        uploaderControl = new SWFUpload({
        upload_url: handler,
        ...
        upload_success_handler: successClientHandler,
        upload_complete_handler: uploadComplete,
        // Button settings

        // Button settings
        button_image_url: "/Content/KaYiLib/Uploader/image/XPButtonNoText_160x22.png",
        button_placeholder_id: buttonID,
        button_width: 60,
        button_height: 20,
        button_text: '<span class="button" style="padding:5px 10px!important;">Add File</span>',
        button_text_style: '.button { font-family: 宋体; font-size: 12pt; color:#ffffff } ',
        button_text_top_padding: 1,
        button_text_left_padding: 5,

        // Flash Settings
        flash_url: "/Content/KaYiLib/Uploader/Referenced/swfupload.swf", // Relative to this file                 
        flash9_url: "/Content/KaYiLib/Uploader/Referenced/swfupload_FP9.swf", // Relative to this file
        custom_settings: {
            upload_target: promptLayer
        },
        // Debug Settings
        debug: false
        });

    return uploaderControl;
}
...
function uploadError(file, errorCode, message) {
    ...
    try {
        switch (errorCode) {
            case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
                try {
                    prompt = "<span class='uploader_error'>上传已取消</span>";
                }
                catch (ex1) {
                    this.debug(ex1);
                }
            ...
            case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
                try {
                    prompt = "<span class='uploader_error'>上传已终止</span>";
                }
                catch (ex2) {
                    this.debug(ex2);
                }
            case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
                prompt = "<span class='uploader_error'>文件大小超出限制</span>";
                break;
            default:
                prompt = "<span class='uploader_error'>" + message + "</span>";
                break;
        }
    ...
    displayPrompt(this.customSettings.upload_target, prompt);
}
...
function uploadProgress(file, bytesLoaded) {
    if (bytesLoaded == undefined) return;
    var prompt = "";
    try {
        ...
        prompt = "<div id='UploadTip'><span class='uploader_filename'>" + file.name + "</span><span class='uploader_prompt'>正在上传,已完成" + percent + "%</span></div>";
...
function displayPrompt(destLayer, prompt) {
    document.getElementById(destLayer).innerHTML = prompt;
}
